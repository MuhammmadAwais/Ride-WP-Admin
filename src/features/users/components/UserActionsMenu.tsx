...
import { MoreVertical, Eye, UserMinus, UserCheck, Trash2, Loader2 } from 'lucide-react';
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (modalType === 'suspend') {
        ...
        toast.success(`${userName} suspended successfully.`);
      } else if (modalType === 'unsuspend') {
        await suspendUser({ userId: numericId, isSuspended: false }).unwrap();
        toast.success(`${userName} unsuspended successfully.`);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="font-roboto text-[14px] text-text-muted leading-relaxed mb-7">
          {isDelete
            ? 'This action will permanently purge this user account and associated history. This cannot be undone.'
            : isUnsuspend
            ? 'This user will regain full access to the mobile application, rides, and clubs.'
            : 'This user will be temporarily suspended from participating in rides, clubs, and marketplace activities.'}
        </p>
